...
class NinjaFile(object):
    ...
    def handle_build_node(self, n):
        ...
        # remove the $( and $) pseudo commands
        if re.match(r'\$[\(\)]',str(n.executor)):
            cmd = re.sub('\$[\(\)]','',str(n.executor))
        ...
        tempfile_match = re.match('(.*)\$\{TEMPFILE\([\"\']',str(n.executor))
        if tempfile_match:
            prefix = tempfile_match.group(1)
        ...
        # ignore anything enclosed by $( and $)
        trimmed_executor = re.sub(r'\$\(.*\$\) *','',str(n.executor))
        needs_scons = (isinstance(n.executor.get_action_list()[0], SCons.Action.FunctionAction)
                       or '(' in trimmed_executor)
        if needs_scons:
            print("WARNING: building {} via SCONS because ninja doesn't know out how to \"{}\"".format(n,n.executor))
        ...
        tool=""
        for tool in trimmed_executor.split(None):
            if tool.startswith('$ICE'):
                continue
            break
        assert(tool)
    ...
    def write(self):
        ...
        ninja.comment('write_vars')
        self.write_vars(ninja)
        ninja.comment('write_rules')
        self.write_rules(ninja)
        ninja.comment('write_builds')
        self.write_builds(ninja)
        ninja.comment('write_regenerator')
